...
//#endif
	...
	SDL_WM_SetCaption("TOPOSYN Demo", 0);
	...
	while(!done)
	{
		...
		if (showMenu)
		{
			...
			if (geom && sample)
			{
				...
				if (imguiButton("Build Buckets"))
				{
					ctx.resetLog();
					if (!sample->handleBuildBuckets())
					{
						showLog = true;
						logScroll = 0;
					}
		...
		if (showLog && showMenu)
		{
			if (imguiBeginScrollArea("Log", 260+20, 10, width - 300 - 250, 200, &logScroll))
				mouseOverMenu = true;
		...
		{
			if (imguiBeginScrollArea("Tools", 10, 10, 260, height-20, &toolsScroll))
				mouseOverMenu = true;
