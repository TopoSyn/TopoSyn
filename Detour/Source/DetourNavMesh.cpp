...
dtStatus dtNavMesh::setPolyAreaId(dtPolyRef ref, unsigned short int areaId)
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
	...
	poly->setAreaId(areaId);
	
	return DT_SUCCESS;
}

dtStatus dtNavMesh::getPolyAreaId(dtPolyRef ref, unsigned short int* resultAreaId) const
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
