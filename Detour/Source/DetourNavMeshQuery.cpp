...
bool dtQueryFilter::passAreaFilter(const dtPolyRef /*ref*/,
									  const dtMeshTile* /*tile*/,
									  const dtPoly* poly,
									  unsigned short int areaId) const
{
	return (poly->getAreaId()==areaId);
}
...
inline bool dtQueryFilter::passAreaFilter(const dtPolyRef /*ref*/,
									  const dtMeshTile* /*tile*/,
									  const dtPoly* poly,
									  unsigned short int areaId) const
{
										 ...
										 dtPolyRef* randomRef, float* randomPt) const
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		if (p->getType() == DT_POLYTYPE_OFFMESH_CONNECTION)//(p->getType() != DT_POLYTYPE_GROUND)
			continue;
		// Must pass filter
...
dtStatus dtNavMeshQuery::findBucketPortalNeighbours(dtPolyRef polyRef,dtPolyRef* portalNeighbour,
	                                                float* leftVert,float* rightVert,
										            const dtQueryFilter* filter,int* portalCount) const
{
	*portalCount = 0;
	int m = 0;
	// Validate input
	...
	m_nav->getTileAndPolyByRefUnsafe(polyRef, &polyTile, &poly);
		...
		for (unsigned int i = poly->firstLink; i != DT_NULL_LINK; i = polyTile->links[i].next)
		{
			...
			// Find edge.
			float va[3], vb[3];
			if (!getPortalPoints(polyRef, poly, polyTile, neighbourRef, neighbourPoly, neighbourTile, va, vb))
				continue;

			portalNeighbour[m]=neighbourRef;
			dtVcopy(&leftVert[m*3],va);
			dtVcopy(&rightVert[m*3],vb);
			++m;
		}
...
dtStatus dtNavMeshQuery::findWay(dtPolyRef startRef, dtPolyRef endRef,
								  const float* startPos, const float* endPos,
								  const dtQueryFilter* filter,unsigned short int areaId,
								  dtPolyRef* path, int* pathCount, const int maxPath) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	...
	*pathCount = 0;
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			if (!filter->passAreaFilter(neighbourRef, neighbourTile, neighbourPoly, areaId))
				continue;
...
void dtNavMeshQuery::getPolyCenter(dtNavMesh* navMesh, dtPolyRef ref, float* center)
{
	center[0] = 0;
	center[1] = 0;
	center[2] = 0;
...
float dtNavMeshQuery::findCityGroundPolys(dtNavMesh* mesh) 
{
    dtPolyRef pref;
	float min,h,Pos[3];
	for (int i = 0; i < mesh->getMaxTiles(); ++i)
	{
		...
		const dtPoly* p = &tile->polys[0];
	    pref = base | (dtPolyRef)0;
	    getPolyCenter(mesh, pref, Pos);
	    this->getPolyHeight(pref, Pos, &h);
		min=h;
		for (int j = 1; j < tile->header->polyCount; ++j)
	     {
		    const dtPoly* p = &tile->polys[j];
	        pref = base | (dtPolyRef)j;
	        getPolyCenter(mesh, pref, Pos);
	        this->getPolyHeight(pref, Pos, &h);
			if (min>h)
				min=h;
		...
		for (int j = 0; j < tile->header->polyCount; ++j)
	     {
             dtPoly* p = &tile->polys[j];
	         pref = base | (dtPolyRef)j;
	         getPolyCenter(mesh, pref, Pos);
	         this->getPolyHeight(pref, Pos, &h);
			 if (min==h)
			   p->type = DT_POLYTYPE_CITY_GROUND;
		 }
    }
	return min;
}
...
dtStatus dtNavMeshQuery::findPolysAroundBucket(dtPolyRef startRef, const float* centerPos, const float radius,
											   const dtQueryFilter* filter,
											   dtPolyRef* resultRef, dtPolyRef* resultParent, float* resultCost,
											   int* resultCount, const int maxResult,
											   dtPolyRef* portalRef, dtPolyRef* portalNeighbour,float* leftVert,float* rightVert,
											   int* portalCount) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	dtAssert(m_openList);
...
dtStatus dtNavMeshQuery::findLocalNeighbour(dtPolyRef startRef, dtPolyRef portRef,
												dtPolyRef* resultRef,int* resultCount, const int maxResult,
												unsigned short int* resultAreaId) const
{
	dtAssert(m_nav);
	dtAssert(m_tinyNodePool);

	dtPolyRef* resultParent=NULL;
	*resultCount = 0;
	...
	while (nstack)
	{
		...
		for (unsigned int i = curPoly->firstLink; i != DT_NULL_LINK; i = curTile->links[i].next)
		{
			...
			m_nav->getPolyArea(neighbourRef,&resultArea);
			unsigned short areaId;
			m_nav->getPolyAreaId(neighbourRef,&areaId);
			if (resultArea == 2)
			{
				*resultAreaId=areaId;
				continue;
