...
bool dtPathCorridor::init(const int maxPath)
{
	...
	m_intermpath = (dtPolyRef*)dtAlloc(sizeof(dtPolyRef)*maxPath, DT_ALLOC_PERM);
	if (!m_path)
		return false;
	m_npath = 0;
	m_mpath = 0;
...
bool dtPathCorridor::optimizePathTopology(dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	...
	dtPolyRef res[MAX_RES];
	int nres = 0;
	//dtStatus status = m_bucMesh->bcFindPath(m_navMesh,*m_bucg,startRef,ag->targetRef,ag->npos,ag->targetPos,linePath,&pathCount);
...
bool dtPathCorridor::addToIntermPath(dtPolyRef ref, const int npath,float* interm_pos)
{
	dtAssert(npath>=0);
	
	for (int i = 0; i < m_mpath; ++i)
	{
		if (ref==m_intermpath[i])
			return false;
	}

	m_intermpath[npath]=ref;
...
void dtPathCorridor::dtGetDistTurns(float* dist, unsigned short* t)
{
	*t=m_nturns;
	*dist=m_dist_walked;
