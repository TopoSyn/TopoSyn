...
void dtCrowd::initDensityMap(int n)
{
	for (int i = 0; i < n; ++i)
	{
		m_mapDensity[i]=0;
...
bool dtCrowd::init(const int maxAgents, const float maxAgentRadius, dtNavMesh* nav,bcBucketMesh* bucMesh)
{
	purge();
	m_navMesh = nav;
	m_bucMesh = bucMesh;
	...
	// Init density map.
	initDensityMap(bucg.m_edges.size()); 
...
int dtCrowd::addAgent(const float* pos, const dtCrowdAgentParams* params)
{
	...
	unsigned char area;
	m_navMesh->getPolyAreaId(ref,&areaId);
	m_navMesh->getPolyArea(ref,&area);
	if (area!=2)
	{
		m_mapDensity[areaId]=m_mapDensity[areaId]+1;
		m_roadCounter[areaId]=m_roadCounter[areaId]+1;
	}
...
bool dtCrowd::requestMoveTarget(const int idx, dtPolyRef ref, const float* pos,BcGraph& bg)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
	if (!ref)
	...
	ag->targetRef = ref;
	dtVcopy(ag->targetPos, pos);
	ag->intermtargetRef = 0;
	ag->intermtarget=false;
	ag->targetPathqRef = DT_PATHQ_INVALID;
	ag->targetReplan = false;
	if (ag->targetRef)
	{
		ag->targetState = DT_CROWDAGENT_TARGET_REQUESTING;
		if (ag->params.familiarity==0)
			requestIntermRoaming(idx, ag->targetRef, ag->targetPos,bg);
		else
			requestIntermMoveTarget(idx, ag->targetRef, ag->targetPos,bg);
	}
	else
		ag->targetState = DT_CROWDAGENT_TARGET_FAILED;
...
bool dtCrowd::requestIntermMoveTarget(const int idx, dtPolyRef ref, const float* pos,BcGraph& bg)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
	if (!ref)
	...
	dtPolyRef* mpath = ag->corridor.getIntermPath();
	const int npath = ag->corridor.getPathCount();
	int mnpath = ag->corridor.getIntermPathCount();
	dtAssert(npath);
	//dtAssert(mnpath);
	...
	dtPolyRef startRef,psartRef;
	dtPolyRef endRef;
	float spos[3];
	float epos[3];
	int areaID=0;
	struct bcLineStep linePath[32];
	...
	{
		startRef=path[0];
		ag->corridor.addToIntermPath(startRef,0,ag->npos);
		endRef=ag->targetRef;
		dtVcopy(spos,ag->npos);
		dtVcopy(epos,ag->targetPos);	

		status = m_bucMesh->bcFindPath(m_navMesh,bg,m_mapDensity,startRef,startRef,ag->targetRef,ag->npos,ag->targetPos,
			                           ag->params.familiarity, ag->params.exploration,linePath,&lineCount,&nturns);
		...
		{
			endRef=linePath[i].interBuc;
			areaID=linePath[i].bcLineRef;
			m_navquery->getPolyCenter(m_navMesh,endRef,epos);

			ag->intermtargetRef=endRef;
			bool state = ag->corridor.addToIntermPath(ag->intermtargetRef,i+1,epos);
			if (state==false)
			{
				ag->targetState = DT_CROWDAGENT_TARGET_FAILED;
				return true;
			}
			dtVcopy(ag->intermtargetPos,epos);
			if (i==0)
			{
				m_mapDensity[areaID]=m_mapDensity[areaID]-1;
			}
			else
			{
				m_mapDensity[areaID]=m_mapDensity[areaID]+1;
				m_roadCounter[areaID]=m_roadCounter[areaID]+1;
				ag->parentLine=areaID;
			}
		}
	}
	else 
	{
		m_mapDensity[ag->parentLine]=m_mapDensity[ag->parentLine]-1;
		startRef=path[npath-1];
		endRef=ag->targetRef;
		dtVcopy(spos,ag->npos);
		dtVcopy(epos,ag->targetPos);
		
		status = m_bucMesh->bcFindPath(m_navMesh,bg,m_mapDensity,mpath[mnpath-2],startRef,ag->targetRef,ag->npos,ag->targetPos,
				                       ag->params.familiarity, ag->params.exploration,linePath,&lineCount,&nturns);
		if (status!=DT_SUCCESS)
			return false;
		endRef=linePath[lineCount-1].interBuc;
		m_navquery->getPolyCenter(m_navMesh,endRef,epos);

		ag->parentLine=linePath[lineCount-1].bcLineRef;
		ag->intermtargetRef=endRef;
		bool state = ag->corridor.addToIntermPath(ag->intermtargetRef,mnpath,epos);
		if (state==false)
			{
				ag->targetState = DT_CROWDAGENT_TARGET_FAILED;
				return true;
		...
		m_mapDensity[ag->parentLine]=m_mapDensity[ag->parentLine]+1;
		m_roadCounter[ag->parentLine]=m_roadCounter[ag->parentLine]+1;
	}

	float d=0.0; unsigned short t=0;
	ag->corridor.dtGetDistTurns(&d,&t);
	if (ag->targetRef)
		ag->targetState = DT_CROWDAGENT_TARGET_REQUESTING;
	else
		ag->targetState = DT_CROWDAGENT_TARGET_FAILED;
...
bool dtCrowd::requestIntermRoaming(const int idx, dtPolyRef ref, const float* pos,BcGraph& bg)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
	if (!ref)
	...
	if (!ag->intermtarget)
	{
		...
		status = m_bucMesh->bcChoosePath(m_navMesh,bg,m_mapDensity,startRef,startRef,ag->targetRef,ag->npos,ag->targetPos,linePath,&lineCount,&nturns);
	...
	else 
	{
		...
		status = m_bucMesh->bcChoosePath(m_navMesh,bg,m_mapDensity,mpath[mnpath-2],startRef,ag->targetRef,ag->npos,ag->targetPos,linePath,&lineCount,&nturns);
		if (status!=DT_SUCCESS)
			return false;
		endRef=linePath[lineCount-1].interBuc;
		m_navquery->getPolyCenter(m_navMesh,endRef,epos);
...
void dtCrowd::updateMoveRequest(const float /*dt*/)
{
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		if (ag->targetState == DT_CROWDAGENT_TARGET_REQUESTING)
		{
			...
			startRef=path[npath-1];
			endRef=ag->intermtargetRef;
			dtVcopy(spos,ag->npos);
			dtVcopy(epos,ag->intermtargetPos);
			
			
			m_navquery->initSlicedFindPath(startRef, endRef, spos, epos, &m_filters[ag->params.queryFilterType]);
			m_navquery->updateSlicedFindPath(MAX_ITER, 0);
			dtStatus status = 0;
			if (ag->targetReplan) 
					{
						// Try to use existing steady path during replan if possible.
						status = m_navquery->finalizeSlicedFindPathPartial(path, npath, &reqPath[0], &reqPathCount, 32);
						
					}
					else
					{
						// Try to move towards target when goal changes.
						status = m_navquery->finalizeSlicedFindPath(&reqPath[0], &reqPathCount, 32);
	
					}

			if (reqPathCount>=MAX_RES)
					break;
			...
			if (!dtStatusFailed(status) && reqPathCount > 0)
			{
				...
				if (reqPath[reqPathCount-1] != ag->intermtargetRef)
				{
					// Partial path, constrain target position inside the last polygon.
					status = m_navquery->closestPointOnPoly(reqPath[reqPathCount-1], epos, reqPos, 0);
					if (dtStatusFailed(status))
						reqPathCount = 0;
	...
	for (int i = 0; i < nqueue; ++i)
	{
		...
		ag->targetPathqRef = m_pathq.request(ag->corridor.getLastPoly(), ag->intermtargetRef,
							ag->corridor.getTarget(), ag->intermtargetPos, &m_filters[ag->params.queryFilterType]);
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		if (ag->targetState == DT_CROWDAGENT_TARGET_WAITING_FOR_PATH)
		{
			...
			else if (dtStatusSucceed(status))
			{
				...
				// Apply results.
				float targetPos[3];
				dtVcopy(targetPos, ag->intermtargetPos);
				...
				if (valid)
				{
					...
					if (res[nres-1] != ag->intermtargetRef)
					{
						// Partial path, constrain target position inside the last polygon.
						float nearest[3];
...
void dtCrowd::updateDensity(dtCrowdAgent* ag)
{
	unsigned short int agentAreaId;
	float agentPos[3];
	dtPolyRef agentRef = 0;
	dtVcopy(agentPos, ag->npos);
	m_navquery->findNearestPoly(ag->npos, m_ext, &m_filters[ag->params.queryFilterType], &agentRef, agentPos);
	m_navMesh->getPolyAreaId(agentRef,&agentAreaId);
	
	if (ag->corridor.getLastPoly() == agentRef) 
		m_mapDensity[agentAreaId]=m_mapDensity[agentAreaId]-1;
}

void dtCrowd::checkPathValidity(dtCrowdAgent** agents, const int nagents, const float dt,BcGraph& bg)
{
...
//			ag->boundary.reset();
		...
		{
			float nearest[3];
			dtVcopy(nearest, ag->npos);
			agentRef = 0;
			m_navquery->findNearestPoly(ag->npos, m_ext, &m_filters[ag->params.queryFilterType], &agentRef, nearest);

			// If the end of the intermediate path is near, replan the new onr.
			if (ag->targetReplanTime > TARGET_REPLAN_DELAY )
			{
				if (ag->corridor.getLastPoly() == agentRef) 
				{
					ag->intermtarget=true;
					requestIntermMoveTarget(idx, ag->targetRef, ag->targetPos,bg);
					//fprintf(fp,"3");
				}
							
				// If the end of the path is near and it is not the requested location, replan.
				/*if ag->corridor.getPathCount() < CHECK_LOOKAHEAD &&
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug,BcGraph& bg)
{
	m_velocitySampleCount = 0;
	...
	checkPathValidity(agents, nagents, dt,bg);
