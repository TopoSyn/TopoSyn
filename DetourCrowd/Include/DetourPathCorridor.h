...
class dtPathCorridor
{
	...
	dtPolyRef* m_path;
	dtPolyRef* m_intermpath;
	int m_npath;
	int m_mpath;
	int m_maxPath;
	float m_dist_walked;
	unsigned short m_nturns;
	float m_prec_pos[3];
...
public:
	...
	bool addToIntermPath(dtPolyRef ref, const int npath,float* interm_pos);
	void dtGetDistTurns(float* dist, unsigned short* t);
	...
	inline dtPolyRef* getIntermPath() { return m_intermpath; }

	/// The number of polygons in the current corridor path.
	/// @return The number of polygons in the current corridor path.
	inline int getPathCount() const { return m_npath; } 
	inline int getIntermPathCount() const { return m_mpath; } 
