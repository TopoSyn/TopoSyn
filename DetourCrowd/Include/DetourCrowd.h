...
/// @ingroup crowd
enum dtCrowdAgentCategory
{
	DT_CROWDAGENT_FAMILIAR,		///< The agent is familiar with environment.
	DT_CROWDAGENT_NOT_FAMILIAR,		///< The agent is not familiar with environment.
};

/// Configuration parameters for a crowd agent. Ajouter l'environnement
/// @ingroup crowd
struct dtCrowdAgentParams
{
	dtCrowdAgentCategory category; 
	unsigned int familiarity;            // Familiarity degree. [Limit: >= 0]
	unsigned int exploration;          // Exploration degree. [Limit: >= 1]
...
struct dtCrowdAgent
{
	...
	bool intermtarget;					///< Flag indicating that the current target is intermediate.
	dtPolyRef targetRef;				///< Target polyref of the movement request.
	dtPolyRef intermtargetRef;			///< Intermediate Target polyref of the movement request.
	unsigned short int parentLine;	    
	float targetPos[3];					///< Target position of the movement request (or velocity in case of DT_CROWDAGENT_TARGET_VELOCITY).
	float intermtargetPos[3];           ///< Intermediate Target position of the movement request
...
class dtCrowd
{
	...
	void updateDensity(dtCrowdAgent* ag);

	void checkPathValidity(dtCrowdAgent** agents, const int nagents, const float dt,BcGraph& bg);
...
public:
	...
	bool init(const int maxAgents, const float maxAgentRadius, dtNavMesh* nav,bcBucketMesh* bucMesh);
	...
	/// The count of agents passed by every road
	std::map<int, int> getRoadCounter() { return m_roadCounter;}
	...
	bool requestMoveTarget(const int idx, dtPolyRef ref, const float* pos,BcGraph& bg);

	/// Submits a new intermediate move request for the specified agent.
	///  @param[in]		idx		The agent index. [Limits: 0 <= value < #getAgentCount()]
	///  @param[in]		ref		The position's polygon reference.
	///  @param[in]		pos		The position within the polygon. [(x, y, z)]
	/// @return True if the request was successfully submitted.
	bool requestIntermMoveTarget(const int idx, dtPolyRef ref, const float* pos,BcGraph& bg);

	bool requestIntermRoaming(const int idx, dtPolyRef ref, const float* pos,BcGraph& bg);
	...
	void update(const float dt, dtCrowdAgentDebugInfo* debug,BcGraph& bg);
